...
- 最近更新：2026-08-02（主 Agent，批 1/批 3 收口后）
- 测试基线：`./run_tests.sh` **547/547 全绿**；`./validate_data.sh` **DATA OK**（仅 P4 美术未交付的路径警告）
...
| D4 | 2026-08-02 | **活性炭口罩**：主 Agent 裁——放营地试剂架可拾取（与 D1 同法，不改 11 条配方）；**氧气瓶**：电解器交互额外灌装 1 个（"氧气可以制备"，按 SPEC-02 §5 字面）；**葡萄糖**：不做来源（用户：食物恢复走篝火进食，葡萄糖不单独显示），items.json 条目保留（UT-G12 不破坏），来源登记赛后清单 | SPEC-05 §1 试剂裁决段与 SPEC-02 §5 道具表同步；电解器输出变化需同步 IT-G13 |
...
B1 氧气 70 提示（D6 新增 sys_oxygen_tutorial）、B2 拾取音效挂载点、B3 unlock_tip 消费、B4 学院驱怪、B5 矿洞呼吸、B6 光合作用、B7 ViewLight 兜底、B8 HUD 语义色、B9 SPEC-08 一行——全部落地。剩余：

| # | 缺口 | 修复方案 | 涉及文件 | 依据 |
|---|---|---|---|---|
| B10 | 中文像素字体未配置 | 需要字体资产（P4）；可先选系统可用的中文等宽字体兜底并记录在案 | `project.godot`（theme font）或各面板 | SPEC-08 §6 |

### C 级：CuSO₄ 池 / 灭火器剧情点（演示不需要，建议登记赛后或低成本补）

| # | 缺口 | 建议 |
|---|---|---|
| C1 | CuSO₄ 溶液池实体（5/s + `warn_cuso4`） | 矿洞加一个蓝色伤害区 Area2D（复用 zone_trigger 模式 + 每帧结算），成本小可做；回归脚本不涉及 |
...
| F6 | 主菜单图鉴门显示空收集（codex 独立场景自建空 Discovery） | 已知限制，MVP 接受；如要修：codex 支持注入共享 Discovery 单例 |
...
3. **可选低成本**：C1 CuSO₄ 伤害池（矿洞蓝色 Area2D + 每帧结算 + `warn_cuso4`），回归脚本不涉及，有余力再做。
