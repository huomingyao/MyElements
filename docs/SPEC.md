...
| 2026-08-02 | v2.0 | 主菜单按钮组下移至「中间偏下」：`main_menu` 的 `MenuBox` 垂直锚点 0.5→0.66（水平居中不变），避开背景人物区；FR-U-06 AC2 与 IT-U06 口径同步（SPEC-01/03/06），暂停菜单保持居中 | 用户 |
...
| 2026-08-03 | v2.3 | 新增 FR-C-10 区域相机钳制与边界幕布：相机随 `zone_changed` 按区钳制（出生草原）、四条边界全高黑幕布（x=-700/-150/600/1375）、黑洞传送瞬间相机 snap + 过场锁输入（`travel_started`/`traveled` 成对）——触碰边缘才切图，此前看不到其他地图；SPEC-01/02/03/06 同步，IT-C10 登记 | 用户 |
| 2026-08-03 | v2.4 | FR-C-10 v2 等宽画布重排（用户拍板取代幕布方案）：四区画布统一 1000px（≥640 视口）紧凑排列（草原 -640..360 / 营地 420..1420 / 矿洞 1480..2480，盐湖不动），屏幕恒被当前区画布铺满——两侧不露黑边，4 条幕布移除；边界 4 黑洞缩为 3 个隐形触发器（x=-670/390/1450，原学院缺口带消失）；黑洞视觉（黑核/紫环）同步移除；白盒图/world.tscn/world.gd 常量/测试坐标全量同步；SPEC-01/02/03/06 同步 | 用户 |
