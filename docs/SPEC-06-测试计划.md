...
| IT-C10 | FR-C-10 | 出生相机边界 == 草原矩形；`zone_changed` 即切换相机边界到对应区域矩形；四区画布等宽 1000px 且无幕布节点；`travel_started` 先于 `traveled`；传送瞬间 `reset_camera_smoothing` 被调用 |
...
| IT-G05 | FR-G-05 | 拖入材料 → 取消回背包不丢失；成功入包 + 弹卡片；**失败不消耗材料**；三种器材选项；背包内 X 键开/关合成台（AC4/AC5 同屏并列，覆盖于 IT-U05） |
...
| IT-U05 | FR-U-05 | 打开时玩家输入被屏蔽；图标缺失显示占位不崩溃；背包打开时按 X 同屏并列开/关合成台（FR-G-05 AC4/AC5） |
| IT-U06 | FR-U-06 | 背包靠左（51%×83%）、合成台靠右（34%×89%）同屏并列不重叠且各自垂直居中、卡片居中（80%×70%）；主菜单按钮组水平居中且垂直中心≈视口 66%（中间偏下）；暂停菜单按钮组居中；死亡画面文案垂直中心锚点 |
