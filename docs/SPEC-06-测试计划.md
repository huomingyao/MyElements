...
| UT-G12 | FR-G-12 | `test_items.gd` | 每个道具效果值读自 balance；装备型不消耗、消耗型 -1；氧气瓶 +50、葡萄糖 +20 |
...
| IT-G10 | FR-G-10 | 矿洞全天/草原仅夜晚生成；接触 8/s；戴口罩 0 伤害；首次接近触发 `warn_co` |
...
覆盖统计：53 条 FR 中 **47 条有自动化测试**，6 条为手工验收——`FR-M-10`（配置面板占位 UI）、`FR-U-02`（主观引导体验）、`FR-B-01..04`（构建/断网/干净机/视频交付）。这六条无法自动化，必须在 H18 按 §7 逐条人工打勾。
...
11. 下矿洞 → 确认氧气加速掉 → 捡 S/CaCO₃ → 硬挨 CO 幽灵一下 → 戴口罩再靠近确认不掉血。
